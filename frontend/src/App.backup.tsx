...
const API = import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8001/api/v1";
...
function App() {
  ...
  async function loadAllVictims(actorId: string): Promise<Victim[]> {
  const pageSize = 100;
  const all: Victim[] = [];

  for (let offset = 0; ; offset += pageSize) {
    const page = await request<Victim[]>(
      `/dashboard/${actorId}/victims?offset=${offset}&limit=${pageSize}`
    );
  ...
  async function loadActor(id:string) {
    ...
    try {
      const [o,c,i,a,v,m]=await Promise.all([
        ...
        loadAllVictims(id),
        request<{rows:{country:string;industry:string;count:number}[]}>(`/dashboard/${id}/matrix`),
      ]);
      setOverview(o); setCountries(c); setIndustries(i); setActivity(a); setVictims(v); setMatrix(m);
    } catch(e){ setError(e instanceof Error?e.message:"Unable to load actor dashboard."); }
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        {page==="dashboard" && overview && <Dashboard overview={overview} profile={actorProfiles[actorId]} countries={countries} industries={industries} activity={activity} matrix={matrix} victims={victims} onVictim={setSelectedVictim} onCountry={setCountryFilter} onIndustry={setIndustryFilter} onAllVictims={()=>setPage("victims")} enrich={enrich} liveSync={liveSync} config={config}/>}
...
function VictimPage(p:{victims:Victim[];countries:Country[];industries:Industry[];search:string;setSearch:(v:string)=>void;country:string;setCountry:(v:string)=>void;industry:string;setIndustry:(v:string)=>void;onVictim:(v:Victim)=>void}) {
  return <div><div className="page-title"><div><div className="crumb">ACTOR-SCOPED INVENTORY</div><h1>Victim Intelligence</h1><p>Search and filter normalized records for the selected threat actor.</p></div></div><div className="filters"><div className="search"><Icon name="search"/><input value={p.search} onChange={e=>p.setSearch(e.target.value)} placeholder="Search victim, country, industry, description…"/></div><select value={p.country} onChange={e=>p.setCountry(e.target.value)}><option value="">All countries</option>{p.countries.map(c=><option key={c.code} value={c.code}>{c.name} ({c.count})</option>)}</select><select value={p.industry} onChange={e=>p.setIndustry(e.target.value)}><option value="">All industries</option>{p.industries.map(i=><option key={i.code} value={i.code}>{i.name} ({i.count})</option>)}</select></div><Panel title={`Victims · ${p.victims.length.toLocaleString()} loaded`} meta="Complete actor-scoped dataset currently loaded; click a row for evidence and provenance"><VictimTable victims={p.victims} onVictim={p.onVictim}/></Panel></div>
}
...
function Actors({actors,current,onSelect,profiles,onSaveProfile}:{actors:Actor[];current:string;onSelect:(id:string)=>void;profiles:ActorProfileMap;onSaveProfile:(id:string,profile:ActorProfile)=>void}){
  ...
  return <div>
    ...
    {selectedActor && <section className="panel actor-profile-editor">
      <div className="panel-head">
        <div>
          <h2>Edit profile · {selectedActor.name}</h2>
          <span>These four fields are maintained in the frontend and shown on the actor dashboard.</span>
        </div>
      </div>
        ...
        <div className="profile-form">
          <label>
            ...
            <textarea
              ...
              placeholder="Enter the threat actor description…"
              rows={4}
            />
          </label>
          ...
          <div className="profile-form-grid">
            <label>
              ...
              <input
                type="date"
                value={profile.first_observed_at}
                onChange={e=>updateProfile("first_observed_at",e.target.value)}
              />
            </label>
            ...
            <label>
              ...
              <input
                type="date"
                value={profile.discovery_date}
                onChange={e=>updateProfile("discovery_date",e.target.value)}
              />
            </label>
          </div>
          ...
          <label>
            ...
            <input
              ...
              placeholder="e.g. DragonForce, DragonForce ransomware"
            />
          </label>

          <div className="profile-actions">
            <button className="primary-button" onClick={saveProfile}>Save Profile</button>
            <span>Saved locally in this browser. Victim collection remains unchanged.</span>
          </div>
        </div>
      </div>
    </section>}
