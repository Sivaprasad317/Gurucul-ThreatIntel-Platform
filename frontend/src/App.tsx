...
async function loadAllVictims(groupId: string): Promise<Victim[]> {
  ...
  return <div className="shell">
    ...
    {profileEditorOpen && overview && (
      <ProfileEditor
        actor={overview.actor}
        profile={actorProfiles[actorId] || EMPTY_ACTOR_PROFILE}
        close={()=>setProfileEditorOpen(false)}
        onSave={(profile)=>{
          saveActorProfile(actorId,profile);
...
function Dashboard({overview,profile,countries,industries,activity,matrix,victims,onVictim,onCountry,onIndustry,onAllVictims,enrich,liveSync,onEditProfile,config}:{overview:Overview;profile?:ActorProfile;countries:Country[];industries:Industry[];activity:Activity;matrix:{rows:{country:string;industry:string;count:number}[]};victims:Victim[];onVictim:(v:Victim)=>void;onCountry:(c:string)=>void;onIndustry:(i:string)=>void;onAllVictims:()=>void;enrich:()=>void;liveSync:()=>void;onEditProfile:()=>void;config:Config|null}) {
  ...
  return <div>
    <section className="actor-hero">
      ...
      <div className="hero-actions">
        <button className="secondary-button small" onClick={onEditProfile}>Edit profile</button>
...
function ProfileEditor({
  ...
  close: () => void;
  onSave: (profile: ActorProfile) => void;
}) {
  const [form,setForm]=useState<ActorProfile>(profile);

  function update<K extends keyof ActorProfile>(
    key: K,
  ...
  return (
    <div className="drawer-backdrop" onClick={close}>
      <aside className="profile-editor-modal" onClick={e=>e.stopPropagation()}>
        <div className="drawer-top">
          <div>
            <div className="crumb">ACTOR PROFILE</div>
          ...
          <div className="profile-form-grid">
            <label>
              ...
              <input
                ...
                onChange={e=>update("first_observed_at",e.target.value)}
              />
            </label>
...
function Actors({
  ...
  async function create(){
    ...
    try{
      ...
      setMessage(`Registered ${created.name}. Select it above to open its dashboard.`);
      setName("");
      setSlug("");
    }catch(e){
  ...
  return <div>
    <div className="page-title">
      <div>
        ...
        <h1>Threat Actors</h1>
        <p>Register actors and select an actor to open its intelligence dashboard.</p>
      </div>
    </div>
    ...
    <div className="actor-cards">
      {actors.map(a=>
        <button
          className={`actor-card ${a.id===current?"selected":""}`}
          key={a.id}
          onClick={()=>onOpenDashboard(a.id)}
        >
